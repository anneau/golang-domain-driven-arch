...
func NewApp() (*App, error) {
	...
	sqsPub, err := outputqueue.NewPublisher(context.Background(), cfg.SQS.QueueURL, cfg.SQS.EndpointURL, cfg.SQS.Region)
	if err != nil {
		return nil, fmt.Errorf("failed to create SQS publisher: %w", err)
	}

	outboxRelay := database.NewOutboxRelay(db, sqsPub, 5*time.Second)

	userUseCases := userusecase.NewUserUseCases(userRepo, userDomainService)
